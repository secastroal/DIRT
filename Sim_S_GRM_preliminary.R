...
# Compile the model
model <- stan_model(file = "Stan/splines_irt_long.stan", verbose = TRUE)
...
outcome.simulation <- foreach(cond = args[1]:args[2], .combine = 'list', .multicombine = TRUE) %:%
  foreach(r = args[3]:args[4], .combine = 'comb', .multicombine = TRUE, 
          .packages = c("rstan", "bayesplot", "splines")) %dopar% {
            ...
            # True states
            B_true <- t(bs(time, df = num_basis, degree = spline_degree, intercept = TRUE)) # B-splines
            a0     <- 0                      # intercept
...
for (i in args[1]:args[2]) {
  ...
  colnames(tmp) <- c("cond",
                     ...
                     "theta.CI",
                     "corrupt")
  write.table(tmp, file = paste0(getwd(), "/Simulation/Sim_S_GRM_cond_", i, ".txt"),
