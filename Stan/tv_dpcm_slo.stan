...
parameters {
  ...
  real a0;                     // intercept
  real<lower=0> tau;           
  real inter;           // time-invariant intercept
...
transformed parameters {
  ...
  //vector<lower=-1, upper=1>[nT] tv_lambda; // time-varying autoregressive effect
  vector[nT] tv_lambda; // time-varying autoregressive effect
  ...
  tv_lambda = a0 * to_vector(time) + to_vector(a * B);
  ...
  for (t in 2:nT) {
    theta[t] = inter + tv_lambda[t] * theta[t - 1] + inno[t];
  }
...
generated quantities {
   ...
   vector[nT] p_var;      // variance of the process 
   ...
   for (t in 1:nT) {
     attractor[t] = inter / (1 - tv_lambda[t]);
     p_var[t]     = sigma2 / (1 - square(tv_lambda[t]));
   }
