...
parameters {
  ...
  real<lower=0> sigma;           // innovations sd
...
transformed parameters {
  ...
  real sigma2;             // innovations variance
  ...
  for (t in 2:nT) {
    theta[t] = tv_int[t] + lambda * theta[t - 1] + inno[t] * sigma;
  }
...
model {
  ...
  for (n in 1:N_obs) {
    ...
    // prob = append_row(rep_vector(0, 1), cumulative_sum(theta[tt_obs[n]] - beta[ii_obs[n], ]));
