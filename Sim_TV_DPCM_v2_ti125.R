  ...
  foreach(r = args[3]:args[4], .combine = 'comb', .multicombine = TRUE, 
          .packages = c("rstan", "bayesplot", "splines")) %dopar% {
            ...
            fit <- sampling(model,                            # Stan model. 
                            ...
                            control = list(adapt_delta=0.95, max_treedepth=12)) # Other parameters to control sampling behavior.
...
for (i in args[1]:args[2]) {
  ...
  colnames(tmp) <- c("cond",
                     ...
                     "corrupt",
                     "efficiency")
  write.table(tmp, file = paste0(getwd(), "/Simulation/Sim_TV_DPCM_v2_ti125_cond_", i, ".txt"),
