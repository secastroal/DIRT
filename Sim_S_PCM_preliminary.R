...
# Compile the model
model <- stan_model(file = "Stan/splines_irt_pcm_long.stan", verbose = TRUE)
...
for (i in args[1]:args[2]) {
  ...
  colnames(tmp) <- c("cond",
                     ...
                     "theta.CI",
                     "corrupt")
  write.table(tmp, file = paste0(getwd(), "/Simulation/Sim_S_PCM_cond_", i, ".txt"),
