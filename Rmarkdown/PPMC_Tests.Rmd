...
#   seed <- seeds[k]
#   nitems <- Nitems[j]
#   rmarkdown::render("Rmarkdown/PPMC_Tests.Rmd", params = list(seed = seed, 
#                                                               nitems = nitems), 
#                     output_file = paste0("PPMC_Tests_", setmodel, "_", seed, "_", nitems))
# }}
```
...
# Responses factor 1
probs.array <- array(NA, dim = c(dim(theta)[1], ceiling(I / 2), K))

for (y in 0:M) {
  probs.array[, , y + 1] <- P.GPCM(y     = y, 
                                   alpha = rep(1, ceiling(I / 2)), 
                                   delta = delta[1:ceiling(I / 2)], 
                                   taus  = taus[1:ceiling(I / 2), ], 
                                   theta = theta[, 1], 
                                   M     = M)
}
...
# Responses factor 2
probs.array <- array(NA, dim = c(dim(theta)[1], I - ceiling(I / 2), K))

for (y in 0:M) {
  probs.array[, , y + 1] <- P.GPCM(y     = y, 
                                   alpha = rep(1, I - ceiling(I / 2)), 
                                   delta = delta[(ceiling(I / 2) + 1):I], 
                                   taus  = taus[(ceiling(I / 2) + 1):I, ], 
                                   theta = theta[, 2], 
                                   M     = M)
}
...
plot(rowSums(data3), type = "l", ylab = "sumscores", xlab = "Time",
     las = 1, ylim = c((I - ceiling(I/2)) - 0.25, (I * K)+ 0.25), lwd = 2)
lines(rowSums(data3[, 1:(ceiling(I/2))]), col = "blue", lwd = 2)
lines(rowSums(as.matrix(data3[, (ceiling(I/2) + 1):I])), col = "darkgreen", lwd = 2)
...
for (y in 0:M) {
  probs.array[, , y + 1] <- P.GPCM(y     = y, 
                                   ...
                                   delta = delta2, 
                                   taus  = taus2, 
                                   theta = theta[(length(theta)/2 + 1):length(theta)], 
                                   M     = M)
}
responses2   <- apply(probs.array, 1:2, function(vec) {which( rmultinom(1, 1, vec) == 1) - 1 })
responses2   <- responses2 + 1 # To fit the PCM in stan, items should be coded starting from 1. 
...
```{r data4_plot}
plot(rowSums(data4[1:(length(theta)/2), ]), type = "l", ylab = "sumscores", xlab = "Time",
     las = 1, xlim = c(1, (length(theta))), lwd = 2, col = "blue", 
     ylim = c(I - 0.25, (I * K)+ 0.25))
lines((length(theta)/2):(length(theta)), 
      rowSums(data4[(length(theta)/2):(length(theta)), ]), 
      col = "darkgreen", lwd = 2)
```

The fifth dataset (Data5) simulates data using the GPCM instead of the PCM for the measurement equation. This means that the discrimination parameters vary across items. The discrimination parameters are 1 for items 1 to `r I - 2`, 0.5 for item `r I - 1` and 1.5 for item `r I`. The simulated data is presented in following Figure:
...
racf <- rep(NA, 6)
for(i in 1:6) {
  racf[i] <- ppmc.racf(fit[[i]], standata[[i]])
...
lpacf <- rep(NA, 6)
for(i in 1:6) {
  lpacf[i] <- ppmc.lpacf(fit[[i]], standata[[i]], sumscores = TRUE)
  mtext(paste0("Data", i), side = 3, line = 3.5)
}
rm(i)
```
...
mssd <- rep(NA, 6)
for(i in 1:6) {
  mssd[i] <- ppmc.mssd(fit[[i]], standata[[i]])
  mtext(paste0("Data", i), side = 3, line = 3.5)
}
rm(i)
```
...
par(mfrow = c(6, 1), oma = c(1, 3, 1, 1), mar = c(1, 4, 2, 1))
for(i in 1:6) {
  ppmc.item.ts(fit[[i]], standata[[i]], quiet = TRUE, items = I - 1)
  mtext(paste0("Data", i), side = 2, line = 4.5, xpd = NA)
}
rm(i)
```
...
it_cortable <- matrix(NA, ncol = I + 1, nrow = 6)
colnames(it_cortable) <- c(paste0("Item", 1:I), "N_misfit")
row.names(it_cortable) <- paste("Data", 1:6)
for(i in 1:6) {
  it_cortable[i, 1:I] <- ppmc.itcor(fit[[i]], 
                                 standata[[i]], 
                                 quiet = TRUE, 
                                 method = "pearson")
}
...
it_cortable[, I + 1] <- apply(it_cortable[, 1:I], 1, function(x) {
  sum(x < 0.05 | x > 0.95)
})
...
it_cor2table <- matrix(NA, ncol = I + 1, nrow = 6)
colnames(it_cor2table) <- c(paste0("Item", 1:I), "N_misfit")
row.names(it_cor2table) <- paste("Data", 1:6)
for(i in 1:6) {
  it_cor2table[i, 1:I] <- ppmc.itcor2(fit[[i]], 
                                 standata[[i]], 
                                 quiet = TRUE, 
                                 method = "pearson")
}
...
it_cor2table[, I + 1] <- apply(it_cor2table[, 1:I], 1, function(x) {
  sum(x < 0.05 | x > 0.95)
})
...
it_cor3table <- matrix(NA, ncol = I + 1, nrow = 6)
colnames(it_cor3table) <- c(paste0("Item", 1:I), "N_misfit")
row.names(it_cor3table) <- paste("Data", 1:6)
for(i in 1:6) {
  it_cor3table[i, 1:I] <- ppmc.itcor3(fit[[i]], 
                                 standata[[i]], 
                                 quiet = TRUE)
}
rm(i)

it_cor3table[, I + 1] <- apply(it_cor3table[, 1:I], 1, function(x) {
  sum(x < 0.05 | x > 0.95)
})
...
ppmc.Q1(fit[[1]], standata[[1]], quiet = TRUE, items = I - 1)
```
...
q1table <- matrix(NA, ncol = I + 1, nrow = 6)
colnames(q1table) <- c(paste0("Item", 1:I), "N_misfit")
row.names(q1table) <- paste("Data", 1:6)
for(i in 1:6) {
  q1table[i, 1:I] <- ppmc.Q1(fit[[i]], standata[[i]], quiet = TRUE)
}
rm(i)

q1table[, I + 1] <- apply(q1table[, 1:I], 1, function(x) {
  sum(x < 0.05 | x > 0.95)
})
...
q1alttable <- matrix(NA, ncol = I + 1, nrow = 6)
colnames(q1alttable) <- c(paste0("Item", 1:I), "N_misfit")
row.names(q1alttable) <- paste("Data", 1:6)
for(i in 1:6) {
  q1alttable[i, 1:I] <- ppmc.Q1.alt(fit[[i]], standata[[i]], quiet = TRUE)
}
rm(i)

q1alttable[, I + 1] <- apply(q1alttable[, 1:I], 1, function(x) {
  sum(x < 0.05 | x > 0.95)
})
...
lpacftable <- matrix(NA, ncol = I + 1, nrow = 6)
colnames(lpacftable) <- c(paste0("Item", 1:I), "N_misfit")
row.names(lpacftable) <- paste("Data", 1:6)
for(i in 1:6) {
  lpacftable[i, 1:I] <- ppmc.lpacf(fit[[i]], standata[[i]], quiet = TRUE)
}
rm(i)

lpacftable[, I + 1] <- apply(lpacftable[, 1:I], 1, function(x) {
  sum(x < 0.05 | x > 0.95)
})
...
sp_plots <- list()
for (i in 1:6) {
  sp_plots[[i]] <- ppmc.Q3(fit[[i]], standata[[i]])
}
q3table <- t(sapply(sp_plots, function(x) x$ppp))
```

```{r q3_plots, fig.height = 6, results='hide'}
ggpubr::ggarrange(sp_plots[[1]]$sp, sp_plots[[2]]$sp, sp_plots[[3]]$sp,
                  sp_plots[[4]]$sp, sp_plots[[5]]$sp, sp_plots[[6]]$sp,
...
sp_plots <- list()
for (i in 1:6) {
  sp_plots[[i]] <- ppmc.OR(fit[[i]], standata[[i]])
}
ortable <- t(sapply(sp_plots, function(x) x$ppp))
```
...
sp_plots <- list()
for (i in 1:6) {
  sp_plots[[i]] <- ppmc.ORDiff(fit[[i]], standata[[i]])
}
ordifftable <- t(sapply(sp_plots, function(x) x$ppp))
```
...
sp_plots <- list()
for (i in 1:6) {
  sp_plots[[i]] <- ppmc.cov.resid(fit[[i]], standata[[i]])
}
residtable <- t(sapply(sp_plots, function(x) x$ppp))
```
...
sp_plots <- list()
for (i in 1:6) {
  sp_plots[[i]] <- ppmc.cov.rediff(fit[[i]], standata[[i]])
}
redifftable <- t(sapply(sp_plots, function(x) x$ppp))
```
...
out_table <- data.frame(seed,
                        model = c("TV-DPCM", "DPCM", "BiDim", "DRIFT", "GPCM", "AR3"),
                        diag.table,
                        acftable,
                        racf,
                        lpacf,
...
out_file <- paste0("Results_", I, "_items.dat")
...
```{r clean}
rm(list = setdiff(ls(), c(lsf.str(), "k", "j", "seeds", "setmodel", "params", "Nitems")))
```
