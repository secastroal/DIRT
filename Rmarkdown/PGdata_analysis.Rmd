...
plot(standata$time[index], sum.fit$theta[, 6][index],
...
#         border = NA,
#         col = rgb(1, 0, 0, 0.25))
lines(standata$time, sum.fit$attractor[, 6],
      col = 1, lwd = 2)
```
...
mcmc_rhat(rhat(fit, pars = c("beta", "theta")))
```
...
```{r acf-plots-pcm, fig.cap = "Autocorrelation Plots of Selected Parameters"}
...
plot(standatapcm$time[index], sum.fit$theta[, 6][index],
...
```{r stanfit-pa}
rdsfile <- "../Fits/PosAwithNA_4items_fixvar"
...
```{r ICC-pa, fig.height = 8, fig.cap = "Item Characteristic Functions for the Mental Unrest Items"}
par(mfrow = c(5, 1), mar = c(2, 4, 2, 2) + 0.1)
...
```{r acf-plots-pcm-na, fig.cap = "Autocorrelation Plots of Selected Parameters"}
...
```{r acf-plots-pcm-pa, fig.cap = "Autocorrelation Plots of Selected Parameters"}
...
```{r ICC-pcm-pa, fig.height = 8, fig.cap = "Item Characteristic Functions for the Mental Unrest Items given the PCM"}
par(mfrow = c(5, 1), mar = c(2, 4, 2, 2) + 0.1)
...
```{r ICC-namu, fig.height = 8, fig.cap = "Item Characteristic Functions for the Mental Unrest Items"}
par(mfrow = c(7, 1), mar = c(2, 4, 2, 2) + 0.1)
...
# Fitting the TV-DPCM to SE items

```{r addNA}
PG_data <- PG_dataComplete[PG_dataComplete$phase >= 5 & 
...
```{r stanfit-se}
rdsfile <- "../Fits/SelfEwithNA_3items_ph5"
...
if (!file.exists(paste0(rdsfile, ".rds"))) {
  ...
  saveRDS(standata, file = paste0(rdsfile, "_data.rds"))
  saveRDS(fit, file = paste0(rdsfile, ".rds"))
  saveRDS(run.time, file = paste0(rdsfile, "_time.rds"))
} else {
  fit      <- readRDS(paste0(rdsfile, ".rds"))
...
plot(standata$time[index], sum.fit$theta[, 6][index],
     type = "l", ylim = c(-4, 7), col = gray(0.5), lwd = 1,
     ylab = "Latent State Disposition", xlab = "Beep Number")
polygon(c(standata$time, rev(standata$time)),
        c(sum.fit$attractor[, 4], rev(sum.fit$attractor[, 8])),
        border = NA,
...
```{r ICC-se, fig.height = 6, fig.cap = "Item Characteristic Functions for the Mental Unrest Items"}
par(mfrow = c(4, 1))

plot.ICC(object = fit, data = standata, range = c(-6, 6), quiet = TRUE,
         item_labels = c("Selflike", "Ashame", "Self-Doubt", "Handle"), 
         col = gray((0:(K - 1))/K))
```

```{r IIF-se, fig.cap = "Item Information Functions of the Items of Mental Unrest"}
plot.IIF(object = fit, data = standata, type = "IIF", 
         item_labels = c("Selflike", "Ashame", "Self-Doubt", "Handle"), 
         col = gray((0:(I-1))/I), range = c(-6, 6))
...
```{r TIF-se, fig.cap = "Test Information Function of the Items of Mental Unrest"}
plot.IIF(object = fit, data = standata, range = c(-6, 6), type = "TIF")
```
