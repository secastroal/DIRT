...
plot.ICC(object = fit, data = standata, range = c(-3, 3), quiet = TRUE,
         ...
         col = gray((0:(K - 1))/K), scale = TRUE, las = 1, lwd = 2)
pos <- legend(2.5, 2.4, c("1", "2", "3"), lty = 1, col = gray((0:(K - 1))/K), 
...
par(mfrow = c(1, 3), mar = c(4, 4, 2, 0) + 0.1)

esttheta <- (sum.fit$theta[, 6]/sqrt(sum.fit$pvar[, 6]))[unique(standata$tt_obs)]
expscores <- rowMeans(E)[unique(standata$tt_obs)] 

plot(sort(esttheta), sort(expscores), type = "l",
     ylim = c(0.9, 3.1), xlim = c(-2.5, 3), las = 1, lwd = 2,
     ylab = "Expected Mean Scores", xlab = "Latent State Disposition")
abline(h = 2, v = 0, lty = 2, col = gray(0.5))
plot(esttheta, na.omit(rowMeans(responses)), 
     pch = 20, las = 1, ylim = c(0.9, 3.1), xlim = c(-2.5, 3),
     ylab = "Observed Mean Scores", xlab = "Latent State Disposition")
plot(rowMeans(E), rowMeans(responses), 
     pch = 20, las = 1, ylim = c(0.9, 3.1), xlim = c(0.9, 3.1),
     ylab = "Observed Mean Scores", xlab = "Expected Mean Scores")
abline(a = 0, b = 1, lty = 2, col = gray(0.5))
mtext("A", side = 3, line = -1.5, at = 0.025, font = 2, cex = 1.3, outer = TRUE, xpd = NA)
mtext("B", side = 3, line = -1.5, at = 0.355, font = 2, cex = 1.3, outer = TRUE, xpd = NA)
mtext("C", side = 3, line = -1.5, at = 0.685, font = 2, cex = 1.3, outer = TRUE, xpd = NA)

```

```{r obs-exp, fig.cap = "Observed vs. Expected Mean Scores for the Last 50 Observations"}
plot(tail(na.omit(rowMeans(responses)), 50), type = "l", las = 1, lwd = 2, 
     lty = 1, col = gray(0.85), xlab = "Observed Beeps",
     ylab = "Mean Scores")
lines(tail(expscores, 50), lty = 2, lwd = 2, col = gray(0.5))
legend("bottomright", c("Observed", "Expected"), 
       col = c(gray(0.85), gray(0.5)), lwd = 2, lty = 1:2)
```
